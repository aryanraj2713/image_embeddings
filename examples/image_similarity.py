...
def main():
    ...
    cosine_matches = find_most_similar(query_embedding, embeddings, image_paths, 
                                     metric='cosine')
    euclidean_matches = find_most_similar(query_embedding, embeddings, image_paths, 
                                        metric='euclidean')
    ...
    for path, score in euclidean_matches:
        print(f"{os.path.basename(path)}: {-score:.3f}")  # Convert back to actual distance
