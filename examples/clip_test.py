...
def find_similar_images(embedder: ImageEmbedder, query_image: str, image_paths: List[str]) -> Tuple[List[str], List[float]]:
    ...
    for name, color in test_images.items():
        ...
        if "square" in name:
            ...
            for i in range(5):
                ...
                outer_pt = (int(center[0] + 50 * np.cos(angle)),
                          int(center[1] + 50 * np.sin(angle)))
                pts.append(outer_pt)
                inner_angle = angle + np.pi / 5
                inner_pt = (int(center[0] + 25 * np.cos(inner_angle)),
